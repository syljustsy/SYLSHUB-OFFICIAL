...
local Window = Library.CreateLib("SYLSHUB UNIVERSAL", "Sentinel")
...
LPsection:NewToggle("Noclip", "Makes you go through walls", function(state)
    if state then
        noclip = true
    else
        noclip = false
...
local SectionAuto = AutoFarm:NewSection("Autofarm configs")
SectionAuto:NewToggle("cashier", "ToggleInfo", function(state)
    if state then
        SectionAuto:toggleCashier()
    else
        SectionAuto:toggleCashier()
    end
end)
SectionAuto:NewToggle("boxer", "ToggleInfo", function(state)
    if state then
        SectionAuto:toggleBoxer()
    else
        SectionAuto:toggleBoxer()
    end
end)
SectionAuto:NewToggle("cook", "ToggleInfo", function(state)
    if state then
        SectionAuto:toggleCook()
    else
        SectionAuto:toggleCook()
    end
end)
SectionAuto:NewToggle("supplier", "ToggleInfo", function(state)
    if state then
        SectionAuto:toggleSupplier()
    else
        SectionAuto:toggleSupplier()
    end
end)
SectionAuto:NewToggle("delivery", "ToggleInfo", function(state)
    if state then
        SectionAuto:toggleDelivery()
    else
        SectionAuto:toggleDelivery()
    end
end)
...
main=Create("Frame",gui,{Name="main", Draggable=true, Visible=false, Active=true, Size=UDim2.new(0,350,0,100), Position=UDim2.new(.335,0,0.02,0), BackgroundColor3=Color3.new(0.098,0.098,0.098)})
